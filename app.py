        ...
        for col in range(cols_per_row):
            ...
            if book_idx < len(popular):
                with cols[col]:
                    ...
                    st.text(popular.iloc[book_idx]['Book-Title']) # Display the Book Title
                    st.text(popular.iloc[book_idx]['Book-Author']) # Display the Author name

# Function to recommed Books

def recommed(book_name):
    index = np.where(pt.index == book_name) [0] [0]
    similar_items = sorted(list(enumerate(similarity_scores[index])), key=lambda X : X[1], reverse=True)[1:6]
    # Lets create empty list and in that list want to populate with the book information
...
if st.sidebar.button("Recommend Me"):
    book_recommend = recommed(selected_book)
    cols = st.columns(5)
